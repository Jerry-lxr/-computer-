...
将在版本1.12.10完善应用功能'''
        ...
        def loading(title:str, fast=0):#加载
            ...
            while 1:
                    ...
                    for jj in range(1, delta+1):
                        ...
                        sleep(0 if LOADING_EASY else randint(4, 10)/240)
                    if jdt >= 100:
                        sleep(0.4)
                        print()
                        break
                    sleep(0.08 if LOADING_EASY else fast)
        ...
        def Get_zhuomianNC():#便携式侦测桌面内存
            global zhuomianNC
            zhuomianNC = 0
            for jjj in zhuomian:
                zhuomianNC += zhuomian[jjj][1] + zhuomian[jjj][2]
        ...
        def AD():#神秘小广告
            def adp(sentence, time=1):
                print(sentence, end='\r')
                sleep(time)
            choice = randint(1, 2)
            match choice:
                case 1:
                    adp(f'shequ.codemao.cn/user/754933482发现更多    还剩3秒  ')
                    adp(f'此系统的浏览器\'不兼容\',可移至其他浏览器    还剩2秒  ')
                    adp(f'shequ.codemao.cn/user/754933482发现更多    还剩1秒  ')
                    adp(f'此系统的浏览器\'不兼容\',可移至其他浏览器    还剩0秒  ')
                case 2:
                    adp(f'盘点那些特别好听的神曲          还剩10秒  ')
                    adp(f'据说99%的人听完都说好          还剩9秒   ')
                    adp(f'说说哪一个才是你心中的top1      还剩8秒  ')
                    adp(f'如果都不是,这个月奶茶我不包了    还剩7秒  ')
                    adp(f'据说你列表第一个是个人           还剩6秒  ')
                    adp(f'赶快艾特出来看一看吧            还剩5秒   ')
                    adp(f'听完在评论区说说你的电量        还剩4秒  ')
                    adp(f'如果一样就交个朋友吧           还剩3秒  ')
                    adp(f'输入sj                        还剩2秒   ',0.7)
                    adp(f'99%的人打出的都是手机          还剩1秒  ',1.1)
                    adp(f'那你打出来是什么呢             还剩0秒  ')
                case _:
                    pass
            print(24 * '  ', end='  \r')
                    ...
                    if choice == '全部下载' or choice == 'all':
                        for a in range(0, len(find)):
                            ...
                            for jjjj in zhuomian:
                                if RJXX[find[a]][0] == zhuomian[jjjj][0]:
                                    bl['F'] = 1
                                    break
                            if 'F' in bl:
                                continue
                            Get_zhuomianNC()
                            if NCXG(zhuomianNC, 3, 1, 4)+NCXG(RJXX[find[a]][2], 3, 1, 4)>NCXG(Ememory[0], Ememory[2], 1, 4):
            ...
            def cn():
                ...
                while 1:
                    ...
                    if URL_HK_code == 'desktop':
                        ...
                        global Cmemory, Dmemory, Ememory
        ...
        class jisuanti:
            ...
            def cn():
                ...
                while 1:
                    match (C := randint(1, 7)):
                        case 1:
                            fn, sn = randint(1, 60), randint(1, 60)
                            r = input(f'{fn}+{sn}=')
                            if r == 'Alt F4':
                                return 'Alt F4退出'
                            if r == 'b':
                        ...
                        case 2:
                            ...
                            if str(fn-sn) == r:
                                ...
                                print(f'{txt[1]}{fn-sn}')
                        case 3:
                            fn, sn = randint(-15, 15), randint(-15, 15)
                            r = input(f'{fn}×{sn}=')
                            if r == 'Alt F4':
                                return 'Alt F4退出'
                            if r == 'b':
                            ...
                            if str(fn*sn) == r:
                                ...
                                print(f'{txt[1]}{fn*sn}')
                        case 4:
                            fn, sn = randint(1, 1000), randint(1, 1000)
                            while(not(fn%sn == 0)):
                                fn, sn = randint(1, 1000), randint(1, 1000)
                            r = input(f'{fn}÷{sn}=')
                            if r == 'Alt F4':
                        ...
                        case 5:
                            fn, sn = randint(1, 20), randint(2, 3)
                            r = input(f'{fn}^{sn}=')
                            if r == 'Alt F4':
                                return 'Alt F4退出'
                            if r == 'b':
                        ...
                        case _:
                            fn, sn = randint(2, 10), randint(1, 200)
...
';':'-.-.-.' , '；':'-.-.-.'}
        ...
        jdtpf = ['!', '?']
        BIOS_password = ''
        Cpan = {'System':{'code':{'about':'ЈΕſſΥ', 'occupy':24*1024}}}
        Dpan = {}
        Epan = {'desktop':[]}
        Cpan_NC = Dpan_NC = Epan_NC = 0
        Jindows_coin_started = False
        Jindows_coin = 0
        browse_history = real_browse_history = []
        num_bomb_inf = [0, 100, 1, 0, 100, 1]
        IsRoot = False
        recycle_bin = []
        computer = [zhuomian, Cmemory, Dmemory, Ememory, IsUserVIP, Jindows_coin_started, Jindows_coin, equipment, browse_history, real_browse_history, BIOS_password, Cpan, Dpan, Epan]
            ...
            while 1:
                ...
                if login_ask == 'a':
                    ...
                    while 1:
                        ...
                        print('1.BIOS开机密码')
                        print('2.恢复出厂设置')
                        print('3.转为'+("电脑端" if equipment_temp == "mobile" else "手机端"))
                        print('4.退出')
                        print('5.保存并退出', end='')
                        match (BIOS_ask := input()):
                            case '1':
                            ...
                            case '2':
                                if input('确定恢复出厂设置(Y代表同意)') == 'Y':
                                    ...
                                    break
                            case '3':
                                equipment_temp = 'computer' if equipment_temp == 'mobile' else 'mobile'
                            case '4':
                                break
                            case '5':
            ...
            while 1:
                ...
                while done:
                    ...
                    if JY_influence:
                        ...
                        JY_influence = False
                    full_instruction = input('执行操作(输入how查看如何操作):')
                    ...
                    if printstyle == '我的世界风':
                        ...
                        elif full_instruction[0] == '/':
                            b = ''
                            for a in full_instruction:
                                b += a
                            full_instruction = b
                            instruction = full_instruction.split(' ')
                        else:
                    ...
                    if (instruction_length := len(instruction)) == 1:
                        if full_instruction == 'how':
                            display_line = True
                            print(f'引号内表示文字, 没有引号则代表参数, 括号内表示提示{HOW}')
                        elif full_instruction == '更新日志' or full_instruction == 'changelog':
                            print(CHANGELOG)
                            display_line = True
                        elif full_instruction == '退出登录' or full_instruction == 'logout':
                            done = done2 = False
                            if user_state == 'administrator':
                                computer = [zhuomian, Cmemory, Dmemory, Ememory, IsUserVIP, Jindows_coin_started, Jindows_coin, equipment, browse_history, real_browse_history, BIOS_password, Cpan, Dpan, Epan]
                        elif full_instruction == '下载' or full_instruction == 'download':
                            has_shop = False
                            for a in zhuomian:
                            ...
                            if has_shop:
                                ...
                                print(f'你需要应用商店才能下载')
                        elif full_instruction == '桌面' or full_instruction == 'desktop':pass
                        else:
                            Successful = False
                            Wrong_Combo = Wrong_Combo_temp + 1
                                ...
                                if instruction[1] == str(sw_num):
                                    ...
                                    if start_software == '我的电脑':
                                        ...
                                        strlong = [len(f'{NCZH(Cpan_NC, 3, 3)[1]}{NCZH(Cpan_NC, 3, 3)[0]}'),
                                            ...
                                            len(f'{NCZH(Epan_NC+zhuomianNC, 3, 3)[1]}{NCZH(Epan_NC+zhuomianNC, 3, 3)[0]}')]
                                        longest = max(strlong)
                                        ...
                                        if choose == '':
                                            ...
                                            while wddn_bool:
                                                match (wddn_ask := input('序号:')):
                                                    ...
                                                    case _:
                                                        print(Fore.RED + '无效输入' + Fore.RESET)
                                                        continue
        ...
        清空历史记录''')
                                        ...
                                        while playing:
                                            ...
                                            if choice == 'Alt F4':
                                                ...
                                                running = True
                                                bomb_low = randint(num_bomb_inf[0]+1, num_bomb_inf[1]-num_bomb_inf[2])
                                                bomb_high = bomb_low + num_bomb_inf[2] - 1
                                                highest = num_bomb_inf[1]-1
                                                lowest = num_bomb_inf[0]+1
                                                while running:
                                                    reply = input(f'{lowest - 1}到{highest + 1}  ')
                                                    if reply == 'Alt F4':
                                                        break
                                                    try:
                                                        reply = int(reply)
                                                        if reply <= highest and reply > bomb_high:
                                                            highest = reply - 1
                                                        elif reply >= lowest and reply < bomb_low:
                                                            lowest = reply + 1
                                                        elif reply <= bomb_high and reply >= bomb_low:
                                                            print('你输了' + (f',炸弹是{bomb_low}到{bomb_high}' if num_bomb_inf[2] != 1 else ''))
                                                            running = False
                                                        if highest - lowest == num_bomb_inf[2]-1:
                                                            print(f'你赢了,炸弹是{bomb_low}' + (f'到{bomb_high}' if num_bomb_inf[2] != 1 else ''))
                                                            if Jindows_coin_started:
                                                                Jindows_coin += 1
                                                                print(f'增加1coin')
                                                            running = False
                                                        ...
                                                        pass
                                                num_bomb_inf[0], num_bomb_inf[1], num_bomb_inf[2] = num_bomb_inf[3], num_bomb_inf[4], num_bomb_inf[5]
                                            elif choice == '2':
                                                reply1 = input('请输入下限值(炸弹值大于下限,默认值0)')
                                                reply2 = input('请输入上限值(炸弹值小于上限,默认值100)')
                                                reply3 = input('请输入炸弹范围值(多少炸弹在一起,默认值1)')
                                                try:
                                                    reply1 = int(reply1)
                                                    if reply1 == 'Alt F4':
                                                        break
                                                    ...
                                                    if reply3 == 'Alt F4':
                                                        break
                                                    if reply2-reply1-1 <= reply3 or reply3 <= 0:
                                                        raise
                                                    choice = input('直接回车一直使用,否则就仅这次使用')
                                                    if choice == 'Alt F4':
                                                        break
                                                    num_bomb_inf = [reply1, reply2, reply3, reply1, reply2, reply3] if choice == '' else [reply1, reply2, reply3, num_bomb_inf[3], num_bomb_inf[4], num_bomb_inf[5]]
                                                except:
                                                    print('输入不正确')
                                            elif choice == '3':
                                                break
                    ...
                    if not Successful:
                        ...
                        if printstyle == '正常':
                            ...
                            print(Fore.RED + f'未知的命令:{full_instruction}, 请检查命令是否存在, 以及您对它是否有使用权限' + Fore.RESET)
                        elif printstyle == '不耐烦':
                            if Wrong_Combo == 0:
